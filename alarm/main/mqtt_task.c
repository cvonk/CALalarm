...
_forwardCoredump(ipc_t * ipc, esp_mqtt_client_handle_t const client)
{
    ...
    asprintf(&priv.topic, "%s/coredump/%s", CONFIG_CALALARM_MQTT_DATA_TOPIC, ipc->dev.name);
...
#ifdef CONFIG_CALALARM_HARDCODED_MQTT_CREDENTIALS
    ESP_LOGW(TAG, "Using mqtt_url from Kconfig");
    mqtt_url = strdup(CONFIG_CALALARM_HARDCODED_MQTT_URL);
#else
...
_getCoredump(ipc_t * ipc, esp_mqtt_client_handle_t const client)
{
    ...
    char * topic;
    asprintf(&topic, "%s/coredump/%s", CONFIG_CALALARM_MQTT_DATA_TOPIC, ipc->dev.name);
...
mqtt_task(void * ipc_void) {
    ...
    _topic.ctrl      = malloc(strlen(CONFIG_CALALARM_MQTT_CTRL_TOPIC) + 1 + strlen(ipc->dev.name) + 1);
    _topic.ctrlGroup = malloc(strlen(CONFIG_CALALARM_MQTT_CTRL_TOPIC) + 1);
    assert(_topic.ctrl && _topic.ctrlGroup);
    sprintf(_topic.ctrl, "%s/%s", CONFIG_CALALARM_MQTT_CTRL_TOPIC, ipc->dev.name);
    sprintf(_topic.ctrlGroup, "%s", CONFIG_CALALARM_MQTT_CTRL_TOPIC);
	...
	while (1) {
		...
		if (xQueueReceive(ipc->toMqttQ, &msg, (TickType_t)(1000L / portTICK_PERIOD_MS)) == pdPASS) {
            ...
            if (subtopic) {
                asprintf(&topic, "%s/%s/%s", CONFIG_CALALARM_MQTT_DATA_TOPIC, subtopic, ipc->dev.name);
            } else {
                asprintf(&topic, "%s/%s", CONFIG_CALALARM_MQTT_DATA_TOPIC, ipc->dev.name);
            }
